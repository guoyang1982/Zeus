package com.gy.sched.server.zookeeper;
...
import com.gy.sched.server.zookeeper.timer.ZkCheckTimer;
...
public class Zookeeper implements ServerContext, Constants {
	 ...
	 * @param machine
	 */
	public void writeClient(long taskId, Machine machine) {
		String taskHostPath = PathUtil.getTaskHostPath(taskId, machine.getIpAddress());
		try {
			zkManager.publishOrUpdateData(taskHostPath, machine.toString(), false);
		} catch (Throwable e) {
			logger.error("[Zookeeper]: writeClient error"
					+ ", taskId:" + taskId 
					+ ", machine:" + machine 
					+ ", taskHostPath:" + taskHostPath, e);
		}
	}
