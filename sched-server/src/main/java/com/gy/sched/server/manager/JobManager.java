package com.gy.sched.server.manager;
...
import com.gy.sched.common.util.*;
...
import com.gy.sched.common.domain.SchedState;
import com.gy.sched.common.domain.remoting.RemoteMachine;
import com.gy.sched.common.domain.result.ResultCode;
import com.gy.sched.common.domain.store.ServerJobInstanceMapping;
import com.gy.sched.common.util.*;
...
//			}
	...
	public Result<Boolean> fireJob(Job job, Date fireTime, String uniqueId, final List<String> machineList) {
		...
		try {
			...
			List<RemoteMachine> remoteMachineList = serverRemoting.getRemoteMachines(job.getAppName());
	...
	private Result<Boolean> tryHandleJobInstanceWithLock(Job job, Date fireTime, String uniqueId, final List<String> machineList) {
        ...
        try {
            ...
            List<RemoteMachine> remoteMachineList = serverRemoting
                    .getRemoteMachines(job.getAppName());
            ...
            if(CommonUtil.isAllJob(job.getType())) {
            	...
            	for(int i = 0 ; i < remoteMachineList.size() ; i ++) {
                	...
                	if (result != null && result.getData()) {
                        ...
                        key.setAppName(remoteMachine.getAppName());
    ...
    private TaskSnapshot fillTaskSnapshot(Job job, JobInstanceSnapshot jobInstanceSnapshot, RemoteMachine remoteMachine) {
    	...
    	try {
			taskSnapshot.setBody(BytesUtil.objectToBytes(SchedState.START));
		} catch (Throwable e) {
