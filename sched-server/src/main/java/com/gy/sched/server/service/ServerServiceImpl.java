package com.gy.sched.server.service;
...
//    private ServerService serverService = clientRemoting.proxyInterface(ServerService.class);
	...
	public Result<Boolean> registerAndCreateJobs(Job job) {
		...
		//List<Job> job = RemotingSerializable.fromJson(jobs, List.class);
		return jobPool.createInternalJob(job);
	}
...
//    	RemoteMachine remoteMachine = InvocationContext.acquireRemoteMachine();
//		return serverRemoting.registerJobs(machine, jobMap, remoteMachine);
//	}
...
//                        	}
                            ...
                            jobInstanceManager.finishJobInstance(
                                    new ServerJobInstanceMapping.JobInstanceKey(taskSnapshot.getJobInstanceId(),
                                    		jobId, jobType, remoteMachine.getAppName(), taskSnapshot.isCompensation()),
...
//
//	@Override
//	public QueryLogResult readLog(long jobId, long instanceId, int pageNumber,
//			int pageSize) {
